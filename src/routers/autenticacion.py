...
from db.models.models import User, UserMostrar, UpdateRequestPassword
...
@router.put("/update/mi_contraseña/")
async def update_password(request: UpdateRequestPassword):
    user = db_users.users_proyect.find_one({"email": request.email})
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Usuario no encontrado")

    try:
        result = db_users.users_proyect.find_one_and_update(
            {"_id": user["_id"]},
            {"$set": {"password": get_password_hash(request.password)}},
            return_document=True
        )
        if result:
            return {"message": "Tu contraseña ha sido cambiada exitosamente"}
        else:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error al actualizar la contraseña")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"El cambio de contraseña no ha funcionado: {e}")